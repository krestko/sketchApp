...
class UserSettingsPage extends Component {
  ...
  handleUserSettingsLinks = () => {
    return this.props.settings.reverse().map((setting) => {
      return (<div key={setting.id}>
        ...
        <Link className='Content-link' to={`/components/${this.props.component.id}/settings/${setting.id}/code`}>{`[{CODE}]`}</Link>
        <Link className='Content-link' to={`/components/${setting.component_id}/settings/${setting.id}`}>[//EDIT]</Link>
        <Link className='Content-link' to={`/components/${setting.component_id}/settings/`} onClick={() => this.props.deleteThis(setting.component_id, setting.id)}>[!DELETE]</Link>
        </div>)
    }
  )}
